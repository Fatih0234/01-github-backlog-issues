...
CARDS = [
    ...
    },
    {
        ...
        "display": "line",
        "sql": (
            f"SELECT calendar_date, cumulative_net_change "
            f"FROM gold_mart_issue_daily_flow "
            f"WHERE {W} AND calendar_date >= CURRENT_DATE - INTERVAL 180 DAY "
    ...
    },
    {
        ...
        "display": "line",
        "sql": (
            f"SELECT calendar_date, rolling_7d_opened, rolling_7d_closed "
            f"FROM gold_mart_issue_daily_flow "
            f"WHERE {W} AND calendar_date >= CURRENT_DATE - INTERVAL 180 DAY "
            f"ORDER BY calendar_date"
        ),
    },
    {
        ...
        "display": "line",
        "sql": (
            f"SELECT calendar_date, rolling_7d_net_change "
            f"FROM gold_mart_issue_daily_flow "
            f"WHERE {W} AND calendar_date >= CURRENT_DATE - INTERVAL 180 DAY "
            f"ORDER BY calendar_date"
        ),
    },
    {
        ...
        "display": "table",
        "sql": (
            f"SELECT calendar_date, opened_count, closed_count, net_change "
    ...
    },
    {
        ...
        "display": "line",
        "sql": (
            f"SELECT year_month, median_closure_age_days, p90_closure_age_days "
            f"FROM gold_mart_issue_closure_age_monthly "
            f"WHERE {W} ORDER BY year_month"
        ),
    },
    {
    ...
    },
    {
        ...
        "display": "bar",
        "sql": (
            f"SELECT weekday_num, weekday_name, issue_count "
    ...
    },
    {
        ...
        "display": "table",
        "sql": (
            f"SELECT issue_number, title, created_at, closed_at, state, "
            f"closure_age_days, author_association, label_group, html_url "
...
def _create_cards(token: str, db_id: int, collection_id: int) -> dict[str, int]:
    ...
    for card in CARDS:
        name = card["name"]
        if name in existing:
            log.info("Card already exists: '%s' id=%d", name, existing[name])
            card_ids[name] = existing[name]
            continue
        ...
        resp.raise_for_status()
        cid = resp.json()["id"]
        log.info("Created card '%s': id=%d", name, cid)
        card_ids[name] = cid
...
def _get_existing_dashcards(token: str, dashboard_id: int) -> list[dict]:
    resp = requests.get(
...
def _wire_cards_to_dashboard(
    ...
    # Keep existing dashcards, append only new ones using temporary negative IDs.
    existing_dashcards = _get_existing_dashcards(token, dashboard_id)
    existing_card_ids = {dc["card_id"] for dc in existing_dashcards if dc.get("card_id")}

    new_dashcards = list(existing_dashcards)
    row = 0
    tmp_id = -1
    ...
    for card in CARDS:
        ...
        if cid in existing_card_ids:
            log.info("Card '%s' already on dashboard, skipping.", card["name"])
        else:
            ...
            log.info("Queued card '%s' (id=%d) at row=%d", card["name"], cid, row)
            tmp_id -= 1
        row += 4
